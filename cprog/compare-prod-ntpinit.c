...
FILE *pre1 = popen("/etc/init.d/ntp status ", "r");

        while (fgets(precheck1, sizeof(precheck1), ls) != 0) {
//        printf("NTP is: %c%c%c%c%c%c%c\n", precheck1[14],precheck1[15],precheck1[16],precheck1[17],precheck1[18],precheck1[19],precheck1[20]);
         char precheck2[50];
   int live = snprintf(precheck2, sizeof(precheck2),  "NTP is: %c%c%c%c%c%c%c", precheck1[14],precheck1[15],precheck1[16],precheck1[17],precheck1[18],precheck1[19],precheck1[20]);
...
FILE *pre2 = popen("/etc/init.d/ntp status ", "r");

        while (fgets(precheck1, sizeof(precheck1), ls) != 0) {
 //      printf("NTP is: %c%c%c%c\n", precheck1[30],precheck1[31],precheck1[32],precheck1[33]);
         char precheck2[50];
   int live = snprintf(precheck2, sizeof(precheck2),  "NTP is in a %c%c%c%c%c%c state", precheck1[30],precheck1[31],precheck1[32],precheck1[33],precheck1[34], \
...
// compare the output string
        ...
        char out3[50];
        int rel;
        strcpy(out2, "NTP is in a failed state");
        strcpy(out3, "running");
        rel = strcmp(out2, precheck2);
        if (rel == 0) {
